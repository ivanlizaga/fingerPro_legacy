...
#' @param VA Boolean to switch between analyzing the SEM or the SD of the sources
#'
...
unmix_R_legacy <- function(data, iter = "short", Means = F, VA = "SEM", seed = 123456L)
{
  set.seed(seed)
  ...
  if (Means == T) {
    ...
    mixture <- inputSample(data[nrow(data), 1:x])
    ...
    mixture <- inputSample(data)
    ###########################################
  }
  
  if (nrow(source) >= ncol(mixture)-1) { #ntracer >= nsources - 1
    ...
    cat("\n", 
        "See below the result/s of the unmixing process using the source variability of the best", 
        iter, "results, notice that the second and third rows of the results are the central value or the average with no correction", 
        "\n", "\n")
    
    print(aggregate(. ~ id, data = r, function(x) c(mean = mean(x), SD = sd(x))))
