...
CB_Method <- function(data, Means = F, seed = 123456L)
{
  ...
  if (Means == F) {
    ...
    df_conc0 <- data[grepl("Cont", names(data))]
    df_conc0 <- cbind(data[1:2], df_conc0)
    
    # Extract the ratios mean
    df_Rat0 <- data[!grepl("Cont", names(data))]
    
    # Remove prefixes from column names
    conc_names <- gsub("Cont_", "", names(df_conc0))
    ...
    if (length(na_columns_conc) > 0) {
      cat("\033[31mNA values detected in the content values in the following column(s):\033[39m\n")
    ...
    conc <- sr[grepl("Cont", names(sr))]# Extract the concentrations
    SDconc <- conc[grepl("D", names(conc))]
    d15N <- sr[!grepl("Cont", names(sr))] # Extract the ratios mean
    ...
    for (ii in 1:(ncol(scalar)/2)) { # Check if you have even number of tracers
      ...
      scalar[ii + ncol(scalar) / 2] <-
        (df_tmp[, ii + ncol(scalar) / 2] * conc[ii])#/sqrt(14)
...
#################################################################################################################################    
    ...
    # conc_names <- gsub("Cont_", "", names(df_conc0))
    # rat_names  <- gsub("R_", "", names(df_Rat0))
    conc_names <- grep("^Conc_", names(df_conc0), value = TRUE)
    rat_names  <- grep("^R_", names(df_Rat0), value = TRUE)
    conc_names <- gsub("Cont_", "", conc_names)
    rat_names  <- gsub("R_", "", rat_names)
    ...
    # Check the number of columns
    if (length(conc_names) == length(rat_names)) {
      cat(sprintf("\033[32mSame number of isotopic ratio and content columns: %d.\033[39m\n", length(conc_names)))
    } else {
      if (length(conc_names) > length(rat_names)) {
        cat(sprintf("\033[37mMore \033[31m\033[1mcontent\033[22m\033[39m\033[37m columns (%d) than \033[31m\033[1misotopic ratio\033[22m\033[39m\033[37m columns (%d).\033[39m\n", 
                    length(conc_names), length(rat_names)))
      } else {
        cat(sprintf("\033[37mMore \033[31m\033[1misotopic ratio\033[22m\033[39m\033[37m columns (%d) than \033[31m\033[1mcontent\033[22m\033[39m\033[37m columns (%d).\033[39m\n", 
                    length(rat_names), length(conc_names)))
      }
    }
    # Check the number of columns
    # if (ncol(df_conc0) == ncol(df_Rat0)) {
    ...
    if (length(na_columns_conc) > 0) {
      cat("\033[31mNA values detected in teh content values in the following column(s):\033[39m\n")
      cat(paste("\033[31m\033[1m", na_columns_conc, "\033[22m\033[39m", sep = ", "), "\n")
      cat("\033[33mPlease consider removing or replacing the NA data.\033[39m\n")
    } else {
      cat("\033[32mNo NA values detected in content columns\033[39m\n")
    ...
    for (ii in 1:(ncol(scalar)/2)) { # Check if you have even number of tracers
      ...
      # Calculate the mean value for each source(source ratio mean -  mixture ratios)* FA content
      scalar[ii] <- (df_tmp[,ii] - mix_ratios[,ii + 1]) * conc[,ii] 
